...
##
## TODO: change tau to 1e-4 and calulcate S0 once again !!
##

```{r}
s0_testing <- read.csv("output/5_4_S0_testing.csv", header = F)
colnames(s0_testing) <- c("S0", "T", "P")
```

```{r}
s0_min <- s0_testing[43,]
```

```{r}
ggplot(s0_testing) + 
    geom_point(aes(S0, T)) +
    geom_hline(yintercept = s0_min$T, color = "blue") + 
    theme_light()
```
...
ggplot(s0_testing) + 
    geom_point(aes(S0, P)) +
    geom_hline(yintercept = s0_min$P, color = "blue") + 
    theme_light()
```
...
S020000 <- read.csv("output/5_4_out_S0=20000.0.csv", header = F)
...
ggplot() + 
    ...
    geom_point(data = S010000, aes(t, P, color = "So=10000"), alpha = 0.2) +
    geom_point(data = S020000, aes(t, P, color = "So=20000"), alpha = 0.2) +
    theme_light()
```
...
ggplot() + 
    ...
    geom_point(data = S010000, aes(t, T, color = "So=10000"), alpha = 0.2) +
    geom_point(data = S020000, aes(t, T, color = "So=20000"), alpha = 0.2) +
    theme_light()
```
